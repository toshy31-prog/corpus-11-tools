"""Stockage local JSON/JSONL avec journal append-only vérifiable.
...
class EventStore:
    """Dépôt institutionnel local composé de ``state.json`` et ``events.jsonl``."""
    ...
    def initialize(
        ...
        state = empty_state(timestamp)
        state["actors"][bootstrap_id] = {
            "id": bootstrap_id,
            "name": bootstrap_name,
            "roles": ["auditor", "registrar"],
            "active": True,
        ...
        self._append_event(
            ...
            entity_id="local",
            details={"bootstrap_roles": ["auditor", "registrar"]},
            at=timestamp,
        )
