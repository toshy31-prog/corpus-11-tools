"""Machine générique de propositions, décisions, recours et pouvoirs bornés."""
...
from .event_store import EventStore, StoreError, normalize_time, parse_time
...
def _role_conflicts(
    roles: set[str],
    forbidden_pairs: set[frozenset[str]] | frozenset[frozenset[str]],
) -> list[str]:
    conflicts: list[str] = []
...
class InstitutionalService:
    ...
    def __init__(
        ...
        *,
        ...
        forbidden_role_pairs: set[frozenset[str]] | frozenset[frozenset[str]],
        max_mandate_duration: timedelta,
        max_temporary_power_duration: timedelta,
    ):
        self.store = store
        self.roles = frozenset(roles)
        self.forbidden_role_pairs = frozenset(forbidden_role_pairs)
        self.max_mandate_duration = max_mandate_duration
        self.max_temporary_power_duration = max_temporary_power_duration

    def _mandate_duration_error(self) -> str:
        return (
            "durée maximale du mandat dépassée: "
            f"{_duration_label(self.max_mandate_duration)}"
        )

    def _temporary_power_duration_error(self) -> str:
        return (
            "durée maximale du pouvoir temporaire dépassée: "
            f"{_duration_label(self.max_temporary_power_duration)} "
            f"({int(self.max_temporary_power_duration.total_seconds() // 3600)} heures)"
        )
    ...
    def _actor(state: dict[str, Any], actor_id: str) -> dict[str, Any]:
        ...
        if not actor or not actor.get("active"):
            raise ProtocolError(f"acteur absent ou inactif: {actor_id}")
        return actor
    ...
    def _require_role(cls, state: dict[str, Any], actor_id: str, role: str) -> dict[str, Any]:
        ...
        if role not in actor["roles"]:
            raise ProtocolError(f"rôle requis pour {actor_id}: {role}")
        return actor
    ...
    def add_actor(
        ...
        if actor_id.startswith("system:"):
            raise ProtocolError("le préfixe system: est réservé")
        if actor_id in state["actors"]:
            raise ProtocolError(f"acteur déjà présent: {actor_id}")
        requested = set(roles)
        if not requested:
            raise ProtocolError("au moins un rôle est requis")
        unknown = requested - self.roles
        if unknown:
            raise ProtocolError(f"rôles inconnus: {', '.join(sorted(unknown))}")
        conflicts = _role_conflicts(requested, self.forbidden_role_pairs)
        if conflicts:
            raise ProtocolError(f"rôles incompatibles: {', '.join(conflicts)}")
        actor = {
            "id": actor_id,
            "name": name,
    ...
    def create_proposal(
        ...
        if not title.strip() or not body.strip():
            raise ProtocolError("titre et contenu sont obligatoires")
        if kind not in {"policy", "mandate_authorization", "temporary_power_authorization"}:
            raise ProtocolError("type de proposition inconnu")
        authorization = None
        if kind == "policy":
            if requested_holder_id or authorized_scopes or not_after:
                raise ProtocolError("une proposition policy ne porte pas d'autorisation exécutable")
        else:
            if not requested_holder_id or not not_after:
                raise ProtocolError("titulaire proposé et date butoir sont obligatoires")
            expected_role = {
            ...
            if not scopes:
                raise ProtocolError("au moins un périmètre autorisable est obligatoire")
            deadline = normalize_time(not_after)
            if parse_time(deadline) <= parse_time(timestamp):
                raise ProtocolError("la date butoir de l'autorisation doit être future")
            authorization = {
    ...
    def record_decision(
        ...
        if not proposal:
            raise ProtocolError(f"proposition inconnue: {proposal_id}")
        if proposal["status"] != "submitted":
            raise ProtocolError(f"proposition non décidable dans l'état {proposal['status']}")
        if proposal["author_id"] == by:
            raise ProtocolError("séparation violée: l'auteur ne peut décider sa proposition")
        if outcome not in {"approve", "reject", "defer"}:
            raise ProtocolError("issue attendue: approve, reject ou defer")
        if not reasons.strip():
            raise ProtocolError("motifs obligatoires")
    ...
    def _authorizing_decision(
        ...
        if not decision:
            raise ProtocolError(f"décision inconnue: {decision_id}")
        if decision["effective_outcome"] != "approve":
            raise ProtocolError("la décision n'autorise pas ou plus cette capacité")
        proposal = state["proposals"].get(decision["proposal_id"])
        if not proposal or proposal.get("kind") != expected_kind:
            raise ProtocolError(f"la décision n'est pas de type {expected_kind}")
    ...
    def grant_mandate(
        ...
        if by == holder_id:
            raise ProtocolError("séparation violée: octroyant et titulaire du mandat identiques")
        start = normalize_time(starts_at or timestamp)
        end = normalize_time(expires_at)
        if parse_time(start) < parse_time(timestamp):
            raise ProtocolError("le mandat ne peut commencer avant son octroi")
        duration = parse_time(end) - parse_time(start)
        if duration <= timedelta(0):
            raise ProtocolError("l'échéance doit être postérieure au début")
        if duration > self.max_mandate_duration:
            raise ProtocolError(self._mandate_duration_error())
        if not scope.strip():
            raise ProtocolError("périmètre du mandat obligatoire")
        authorization = proposal["authorization"]
        if authorization["requested_holder_id"] != holder_id:
            raise ProtocolError("titulaire différent de celui approuvé")
        if scope.strip() not in authorization["authorized_scopes"]:
            raise ProtocolError("périmètre du mandat absent de l'autorisation approuvée")
        if parse_time(end) > parse_time(authorization["not_after"]):
            raise ProtocolError("échéance au-delà de la date butoir approuvée")
    ...
    def exercise_mandate(
        ...
        if not mandate:
            raise ProtocolError(f"mandat inconnu: {mandate_id}")
        if mandate["holder_id"] != by:
            raise ProtocolError("seul le titulaire peut exercer le mandat")
        self._require_role(state, by, "mandate_holder")
        if mandate["status"] != "active":
            raise ProtocolError(f"mandat non exerçable: {mandate['status']}")
        if not action.strip():
            raise ProtocolError("action exercée obligatoire")
        self.store.commit(
            state,
            by,
            "mandate_exercised",
    ...
    def revoke_mandate(
        ...
        if by != mandate["grantor_id"] and "auditor" not in actor["roles"]:
            raise ProtocolError("révocation réservée à l'octroyant ou à un auditeur")
        if mandate["status"] not in {"active", "scheduled", "suspended"}:
            raise ProtocolError(f"mandat déjà clos: {mandate['status']}")
    ...
    def create_appeal(
        ...
        if decision["appeal_status"] != "none":
            raise ProtocolError("un recours a déjà été exercé contre cette décision")
        if not grounds.strip():
            raise ProtocolError("moyens du recours obligatoires")
    ...
    def resolve_appeal(
        ...
        if not appeal:
            raise ProtocolError(f"recours inconnu: {appeal_id}")
        if appeal["status"] != "open":
            raise ProtocolError("recours déjà clos")
        decision = state["decisions"][appeal["decision_id"]]
        if by in {appeal["claimant_id"], decision["decision_maker_id"]}:
            raise ProtocolError("séparation violée: le réviseur est partie ou auteur de la décision")
        if resolution not in {"confirm", "remand", "reverse"}:
            raise ProtocolError("résolution attendue: confirm, remand ou reverse")
        if not reasons.strip():
            raise ProtocolError("motifs de résolution obligatoires")
        appeal.update(
            {
                "status": "resolved",
                "reviewer_id": by,
    ...
    def grant_temporary_power(
        ...
        if by == holder_id:
            raise ProtocolError("séparation violée: l'octroyant ne peut recevoir le pouvoir")
        end = normalize_time(expires_at)
        duration = parse_time(end) - parse_time(timestamp)
        if duration <= timedelta(0):
            raise ProtocolError("l'échéance du pouvoir doit être future")
        if duration > self.max_temporary_power_duration:
            raise ProtocolError(self._temporary_power_duration_error())
        ...
        if not label.strip() or not normalized_capabilities:
            raise ProtocolError("libellé et au moins une capacité sont obligatoires")
        authorization = proposal["authorization"]
        if authorization["requested_holder_id"] != holder_id:
            raise ProtocolError("titulaire différent de celui approuvé")
        outside = set(normalized_capabilities) - set(authorization["authorized_scopes"])
        if outside:
            raise ProtocolError(f"capacités non approuvées: {', '.join(sorted(outside))}")
        if parse_time(end) > parse_time(authorization["not_after"]):
            raise ProtocolError("échéance au-delà de la date butoir approuvée")
    ...
    def exercise_temporary_power(
        ...
        if not power:
            raise ProtocolError(f"pouvoir inconnu: {power_id}")
        if power["holder_id"] != by:
            raise ProtocolError("seul le titulaire peut exercer ce pouvoir")
        self._require_role(state, by, "emergency_holder")
        if power["status"] != "active":
            raise ProtocolError(f"pouvoir éteint: {power['status']}")
        if capability not in power["capabilities"]:
            raise ProtocolError("capacité hors périmètre")
        if not action.strip():
            raise ProtocolError("action exercée obligatoire")
        self.store.commit(
            state,
    ...
    def revoke_temporary_power(
        ...
        if power["status"] not in {"active", "suspended"}:
            raise ProtocolError(f"pouvoir déjà éteint: {power['status']}")
    ...
    def separation_audit(self, as_of: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            metadata = self.store.artifact_metadata
            return {
                "ok": False,
        ...
        for actor_id, actor in state["actors"].items():
            ...
            for conflict in _role_conflicts(roles, self.forbidden_role_pairs):
        ...
        for role in sorted(self.roles - {"registrar"}):
            if not any(
