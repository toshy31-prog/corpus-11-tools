...
def mutate_and_require_failure(old: str, new: str, label: str) -> None:
    with tempfile.TemporaryDirectory() as raw:
        repo = Path(raw) / "repo"
        shutil.copytree(SOURCE_REPO, repo, ignore=shutil.ignore_patterns(".git", "node_modules", ".next", "__pycache__"))
...
def test_ci_pinning_rejects_floating_inputs() -> None:
    ...
    mutate_and_require_failure("python-version: '3.12.11'", "python-version: '3.12'", "floating Python minor")
    mutate_and_require_failure("package-manager-cache: false", "package-manager-cache: true", "implicit package-manager cache")
    mutate_and_require_failure("@openai/codex@0.137.0", "@openai/codex@latest", "floating Codex CLI")
