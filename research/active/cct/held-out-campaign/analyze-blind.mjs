...
function summarize(worlds) {
  ...
  for (const world of worlds) for (const run of world.runs) {
    ...
    if (Number.isFinite(run.meanPredictionAbsoluteError)) target.predictionAbsoluteErrors.push(run.meanPredictionAbsoluteError);
  }
  for (const target of Object.values(byLabel)) {
    const values = target.predictionAbsoluteErrors;
    target.meanOfWorldMeanPredictionAbsoluteErrors = values.length
      ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
    delete target.predictionAbsoluteErrors;
  }
  return byLabel;
}
...
await writeFile(join(here, "runs", "public", "blind-verdict.json"), `${JSON.stringify(verdict, null, 2)}\n`, "utf8");
