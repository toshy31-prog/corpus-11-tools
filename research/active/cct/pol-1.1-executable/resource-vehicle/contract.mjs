export function validateResourceVehicle(spec) {
  ...
  for (const field of ["legal_fiscal_host_required", "local_operator_retains_service_authority", "separate_financial_custody_and_independent_review", "affected_people_recourse_required", "public_aggregate_disbursement_ledger_required", "personal_data_or_beneficiary_lists_forbidden", "funds_cannot_buy_coercive_or_discriminatory_capacity", "funding_cannot_condition_vital_access_or_service_authority"]) require(governance[field] === true, `governance.${field}`);
  for (const field of ["existing_service_and_local_need_required", "written_itemized_continuity_plan_required", "payment_to_local_operator_or_supplier_under_local_authorization", "no_exclusive_supplier_or_platform_for_vital_continuity", "no_substitution_for_ordinary_public_duty_without_public_justification", "repair_fund_reserved_before_start", "stop_and_remedy_condition_before_disbursement"]) require(rules[field] === true, `disbursement_rules.${field}`);
