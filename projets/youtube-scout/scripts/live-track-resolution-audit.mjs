...
function competingArtistsFromProjection(
  ...
  /*
   * Les variantes déjà classifiées comme orthographiques
   * ou variantes de crédit sont explicitement NON rivales.
   */
  ...
  for (
    ...
    if (
      issue.type === "orthographic_variant" ||
      issue.type === "credit_name_variant"
    ) {
      for (const raw of issue.candidates || []) {
        const name =
          typeof raw === "string"
            ? raw
            : raw?.name;
  ...
  for (
    ...
    if (
      credit.role === "channel_hint" &&
      name &&
      !preferred.has(k) &&
      !nonCompetingVariants.has(k)
    ) {
  ...
  for (
    ...
    if (
      issue.type === "multiple_primary_candidates"
    ) {
      for (
        const candidate of
        issue.candidates || []
...
function applyLiveEvidenceGate({
  ...
  /*
   * Une contradiction initiale réelle exige davantage
   * qu'un seul fournisseur soutenant le favori.
   *
   * L'absence de résultat pour le rival n'est pas une
   * preuve négative d'identité.
   */
  if (
    (expected.competingArtists || [])
      .length > 0 &&
...
async function main() {
  ...
  console.log(
    discogsToken
      ...
      : "Discogs token: absent — Discogs sera ignoré"
  );
  ...
  console.log(
    "Aucune écriture dans le graphe Scout."
  );
  ...
  const report = {
    ...
    mode:
      "read_only_live_resolution_audit",
  ...
  const filename =
    path.join(
      OUTPUT_DIR,
      "live-track-resolution-audit.json"
    );
