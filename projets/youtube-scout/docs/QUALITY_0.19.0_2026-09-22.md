...
Correctif préparé sur une copie isolée, sans modification du service personnel pendant son développement. Activation autorisée puis effectuée le 22 septembre 2026 à 14:54 (Europe/Paris). Validation musicale finale laissée à l’utilisateur, à sa demande. Aucun accès Google ni fournisseur musical réel utilisé pour ces tests.

### Activation réobservée

- Processus Scout 0.18 PID 137078 remplacé par 0.19 PID 321613, même répertoire, exécutable et environnement ; aucune valeur de secret affichée.
- Identité des fichiers avant application comparée au manifeste : aucune modification concurrente écrasée. Copie du code précédent conservée dans `qa/prepared-0.19.0/source-before-2026-09-22T12-54-30.564Z.tar.gz`.
- `/api/health` confirme `0.19.0` et `ephemeral_per_departure`. Empreintes des six modules client modifiés servies par HTTP identiques à celles préparées et testées.
- Contexte éphémère de contrôle créé, graphe vide vérifié puis contexte fermé. Aucun fournisseur externe sollicité, aucun onglet utilisateur rechargé.
- Archive historique et stockage personnel sauvegardés dans `.data/backups/pre-0.19.0-2026-09-22T12-54-30.564Z-{0,1}.json`, permissions 600. Empreintes avant/après identiques pour les deux fichiers. Le stockage navigateur n’a pas été modifié par le déploiement.
- Reçu local : `qa/prepared-0.19.0/activation.json`. La réobservation établit l’activation et l’ouverture technique, pas la qualité musicale finale.
